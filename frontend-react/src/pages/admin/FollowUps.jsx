...
import { followUpsApi, usersApi } from "../../api/resources";
...
function buildQuery(view, page, assignedTo) {
...
export default function FollowUps() {
  ...
  const refresh = useCallback(async () => {
    setItems(null);
    try {
      const { items: rows, pagination: p } = await followUpsApi.list(buildQuery(view, page, assignedTo));
      setItems(rows);
      setPagination(p);
    } catch (err) {
      setListError(err.message);
    }
  }, [view, page, assignedTo]);
  ...
  const columns = [
    {
      ...
      render: (fu) => (
        <>
          <a className="table-cell-primary" href={`/admin/leads/${fu.leadId}`}>{fu.leadName || "(no name)"}</a>
          <div className="table-cell-muted text-xs">{fu.leadPhone || ""}</div>
        </>
      ),
    },
    { key: "assigned", label: "Assigned to", render: (fu) => fu.assignedToName || userById.get(String(fu.assignedTo))?.name || `#${fu.assignedTo}` },
    ...
    <>
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">Every scheduled follow-up across your client — reschedule, complete, or cancel from here.</p>
        </div>
      </div>
      ...
      <div className="card mb-4">
        <div className="card-body flex gap-3" style={{ flexWrap: "wrap", alignItems: "flex-end" }}>
          ...
          <div className="field" style={{ minWidth: 180, marginBottom: 0 }}>
            ...
            <select className="select" id="f-assigned" value={assignedTo} onChange={(e) => { setAssignedTo(e.target.value); setPage(1); }}>
      ...
      {modal ? (
        <ScheduleFollowUpModal
          ...
          assignableUsers={modal.assignableUsers}
