...
/**
 * §F: "If a field requires a custom definition but no valid tenant field
 ...
 * tenant's own active custom field definitions — checked here at
 * mapping-save time, not just hoped for at ingestion time.
 */
async function assertValidCrmFieldKey(tenantId, crmFieldKey) {
  if (CORE_FIELD_KEYS.has(crmFieldKey)) return;
  const defs = await customFieldModel.list(tenantId, { includeInactive: false });
  if (!defs.some((d) => d.field_key === crmFieldKey)) {
    throw httpError(
      `"${crmFieldKey}" is not a core field (name/phone/email) or an active custom field for this tenant. Create the custom field first.`,
      400,
...
async function list(tenantId, metaFormId) {
  return metaFormFieldMappingModel.listForTenant(tenantId, metaFormId);
}

async function create(tenantId, body) {
  const clean = validateMappingInput(body);
  await assertValidCrmFieldKey(tenantId, clean.crmFieldKey);
  try {
    return await metaFormFieldMappingModel.create(tenantId, clean);
  } catch (err) {
...
async function update(tenantId, id, body) {
  ...
  await assertValidCrmFieldKey(tenantId, crmFieldKey);
  const updated = await metaFormFieldMappingModel.update(tenantId, id, { crmFieldKey });
  if (!updated) throw httpError("Mapping not found.", 404);
  return updated;
}

async function remove(tenantId, id) {
  const removed = await metaFormFieldMappingModel.remove(tenantId, id);
